...
- [ ] **ICON-01**: User sees an SVG sun icon in light mode and an SVG moon icon in dark mode in the header (icon shows current state).
...
- [ ] **ICON-04**: User on mobile or with a trackball hits a tap target ≥ 44×44 CSS-px regardless of the icon's intrinsic 24×24 viewBox.
- [ ] **ICON-05**: User with `prefers-reduced-motion` set sees no rotation/fade transition; user without reduced-motion sees a ≤ 150 ms transition wrapped in `@media (prefers-reduced-motion: no-preference)`.
